...
def run_experiment(
    ...
    if h_data.wrong_hypothesis is not None:
        # invoke the agent with the negative (synthetically generated) hypothesis
        # note: the dataset may not contain a wrong hypothesis, in which case the step is skipped.
        # this case comes up when evaluating model performance on the non-verifiable hypotheses, since they are already wrong by default
...
if __name__ == "__main__":
    ...
    """
    complexity in:
    - hypothesis index level
    - hyothesis level
    - agent configuration level
    
    - no reason we can't run this in parallel on a thread level--all resources are independant
    - core failure mode is that we get a limit on the number of open files--this is due to a memory leak.
    """
    ...
    # experiments.extend(get_reasoning_react_experiments()) # ignore these for now
