...
REPO_BASE_DIR = "/home/ubuntu/DSWizard"
...
def plot_type_i_ii_error_per_pub_type():
    logs_directory_path = os.path.join(REPO_BASE_DIR, "logs_short_2_NV")
    ...
    print("DEBUG:", results['clean_final_answer'].value_counts())
    ...
    # for each pub type, and agent type, count the number of samples where the hypothesis is not NV, and the final answer is NV (which is all cases, here)
    fp_count = results[results['clean_final_answer'] != 'not verifiable'].groupby(['pub_type', 'agent_type']).size().reset_index(name='fp_count')
    ...
    # for each pub type, and agent type, count the number of samples which hypothesis is not nv, and the final answer is **not** NV (which is in none of the cases, here)
    fn_count = results.groupby(['pub_type', 'agent_type']).size().reset_index(name='fn_count').assign(fn_count=0)
    # for each pub type, and agent type, count the total number of samples
    total_count = results.groupby(['pub_type', 'agent_type']).size().reset_index(name='total_count')
    # merge the two dataframes
